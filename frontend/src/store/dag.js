...
export const useDAGStore = defineStore('dag', () => {
    ...
    const execution = computed(() => currentRunId.value == null ? null : (runs.value[currentRunId.value] || null));
    let ws = null;
    let reconnectTimer = null;
    let closedByUs = false;
    let clockTimer = null;
    function upsertRun(d, opts = {}) {
        runs.value[d.runId] = d;
        // 只在首次收到数据或用户主动执行时切换查看对象，
        // 回看历史时其它运行的实时推送不会打断当前查看
        if (opts.select || currentRunId.value == null)
            currentRunId.value = d.runId;
        refreshRunList(d);
    }
    function refreshRunList(latest) {
        if (latest) {
            ...
            if (i >= 0)
                runList.value[i] = summary;
            else
                runList.value.unshift(summary);
